...
func MulMatVec(alpha float64, transA bool, a Matrix, x []float64, incx int, y []float64, incy int) {
	switch a := a.(type) {
	case *CSR:
		csrMulMatVec(alpha, transA, a, x, incx, y, incy)
	case *DOK:
		dokMulMatVec(alpha, transA, a, x, incx, y, incy)
	default:
